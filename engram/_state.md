...
- **T-016** rename trafficMe → NetLeak (toda la app) — hecho, pytest verde, build OK.
- **T-017** migración de historial al actualizar — hecho, 4 tests nuevos verdes (56 total).
- **T-018** landing pixel-perfect del diseño `NetLeak Landing.dc.html` — markup extraído byte a byte, runtime portado (detección SO/contadores/reveal/hover), build+lint verdes.
**Bloqueos:** ninguno
**Próximo paso sugerido:** validación visual de la landing (npm run dev); QA Strong (T-016/T-017/T-018); el PO renombra el repo en GitHub a `NetLeak`; luego PRs encadenados T-015→T-018.
